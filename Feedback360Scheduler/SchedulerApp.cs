    ...
    public partial class SchedulerApp : Form
    {
        ...
        public void SendReminder1Email()
        {
            try
            {
                ...
                //LookUp.WriteProcessingInformation("starting Reminder 1*****************\r\n");
                //LookUp.WriteProcessingInformation("Total Records =" + dtReminder1.Rows.Count.ToString()+"\r\n");
                ...
                if (dtReminder1.Rows.Count > 0)
                {
                    for (int i = 0; i < dtReminder1.Rows.Count; i++)
                    {
                        ...
                        //LookUp.WriteProcessingInformation("Sending Email to  =" + dtReminder1.Rows[i]["CandidateEmail"].ToString() + "\r\n");
                        ...
                        //LookUp.WriteProcessingInformation("Email sent to  =" + dtReminder1.Rows[i]["CandidateName"].ToString() + "\r\n");
            ...
            try
            {
                ...
                if (dtReportAvaliable.Rows.Count > 0)
                {
                    for (int i = 0; i < dtReportAvaliable.Rows.Count; i++)
                    {
                        ...
                        urlPath += "?" + CreateEncryptedQueryString(loginid, password, accountcode);
            ...
            try
            {
                ...
                if (dtParticipantReminder1.Rows.Count > 0)
                {
                    for (int i = 0; i < dtParticipantReminder1.Rows.Count; i++)
                    {
                        ...
                        string accountcode = dtParticipantReminder1.Rows[i]["Code"].ToString();
                        string loginid = dtParticipantReminder1.Rows[i]["LoginID"].ToString();
                        string password = dtParticipantReminder1.Rows[i]["Password"].ToString();
            ...
            try
            {
                ...
                if (dtParticipantReminder2.Rows.Count > 0)
                {
                    for (int i = 0; i < dtParticipantReminder2.Rows.Count; i++)
                    {
                        ...
                        string accountcode = dtParticipantReminder2.Rows[i]["Code"].ToString();
                        string loginid = dtParticipantReminder2.Rows[i]["LoginID"].ToString();
                        string password = dtParticipantReminder2.Rows[i]["Password"].ToString();
        ...
        public static string CreateEncryptedQueryString(string userName, string password, string accountCode)
        {
            ...
            return System.Web.HttpUtility.UrlEncode(EnCryptDecrypt.CryptorEngine.Encrypt(strQS, true));
        }
    }
}
