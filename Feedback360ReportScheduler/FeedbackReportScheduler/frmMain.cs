...
namespace FeedbackReportScheduler
{
    public partial class frmMain : Form
    {
            ...
            try
            {
                ...
                if (dtParticipant != null && dtParticipant.Rows.Count > 0)
                {
                    for (int i = 0; i < dtParticipant.Rows.Count; i++)
                    {
                        ...
                        HandleException("SubmitCount:"+submitCount.ToString());
                        HandleException("UserID:" + dtParticipant.Rows[i]["UserID"].ToString());
                        GetDetailFromTargetPersonID(dtParticipant.Rows[i]["UserID"].ToString());
                        //GetDetailFromTargetPersonID("685");
                        HandleException("strTargetPersonID:" + strTargetPersonID);
                        HandleException("strAccountID:" + strAccountID);
                        HandleException("strProjectID:" + strProjectID);
                        HandleException("strReportName:" + strReportName);
                        HandleException("1-submitCount:" + strSubmitCount);
                        HandleException("2-submitCount:" + submitCount);
                        if (submitCount != Convert.ToInt32(strSubmitCount))
                        {
                            HandleException("Inside-submitCount:" + submitCount);
                            if (strTargetPersonID != string.Empty && strAccountID != string.Empty && strProjectID != string.Empty && strReportName != string.Empty)
                            {
                                HandleException("Inside-strTargetPersonID:" + strTargetPersonID);
                                //Generate Report
                                btnExport("");
                                HandleException("After-strTargetPersonID:" + strTargetPersonID);
        ...
        protected void btnExport(string dirName)
        {
            try
            {
                ...
                if (strReportType == "1")
                {
                    ...
                    if (dtreportsetting != null && dtreportsetting.Rows.Count > 0)
                    {
                        HandleException(" dtreportsetting.Rows.Count:" + dtreportsetting.Rows.Count);
                        ...
                        HandleException(" targetradarname:" + targetradarname);
                    }
        ...
        public void HandleException(string ex)
        {
            ...
            string str = "Error Occured on: " + DateTime.Now + Environment.NewLine + "," +
                        "Error Application: Feedback 360 - UI" + Environment.NewLine + "," +
                        "Error Function: " + ex + Environment.NewLine + ",";
