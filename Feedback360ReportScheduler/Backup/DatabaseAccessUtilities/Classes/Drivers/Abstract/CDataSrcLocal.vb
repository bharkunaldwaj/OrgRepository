...
#Region "Abstract/Virtual (Driver-Specific)"
    ...
    Public Overloads Function Parameter(ByVal i As CNameValue) As IDbDataParameter
        Return Parameter(i.MarkerName, i.Value)
    End Function
...
#Region "NonQuery"
    ...
    End Function
    Public Overloads Function ExecuteNonQuery(ByVal sql As String, ByVal txOrNull As IDbTransaction) As Integer
        Return ExecuteNonQuery(New CCommand(sql, txOrNull))
    End Function
    Public Overloads Function ExecuteNonQuery(ByVal spName As String, ByVal parameterValues As Object(), ByVal txOrNull As IDbTransaction) As Integer
        Return ExecuteNonQuery(New CCommand(spName, parameterValues, txOrNull))
    End Function
    Public Overloads Function ExecuteNonQuery(ByVal spName As String, ByVal pNamesAndValues As CNameValueList, ByVal txOrNull As IDbTransaction) As Integer
        Return ExecuteNonQuery(New CCommand(spName, pNamesAndValues, txOrNull))
    End Function
    Protected Overloads Function ExecuteNonQuery(ByVal cmd As IDbCommand) As Integer
        If LOGGING Then Log(cmd)
        Dim closeConnection As Boolean = IsNothing(cmd.Transaction)
...
#Region "Insert"
    ...
    Public Function ExecuteInsert(ByVal cmd As IDbCommand) As Integer
        ...
        Try
            ...
            If TypeOf Me Is COracleClient Then cmd.CommandText = "SELECT Max(PrimaryKeyName) FROM TableName"
...
#Region "Command Overloads"
    ...
    Protected Function Command(ByVal cmd As CCommand) As IDbCommand
        With cmd
            ...
            'Timeout
            If .Timeout <> Integer.MinValue Then dbCmd.CommandTimeout = .Timeout
...
#Region "Parameters"
    ...
    Private Function DiscoverSpParameterSet(ByVal spName As String, ByVal includeReturnValueParameter As Boolean) As IDbDataParameter()
        ...
        If Not includeReturnValueParameter And Not TypeOf Me Is CMySqlClient Then
            If cmd.Parameters.Count > 0 Then cmd.Parameters.RemoveAt(0)
        End If
...
#Region "Sql - Private"
    Private Function InsertCmd(ByVal tableName As String, ByVal pKeyName As String, ByVal insertPk As Boolean, ByVal data As CNameValueList, ByVal txOrNull As IDbTransaction) As IDbCommand
    ...
    Private Function DeleteWhereCmd(ByVal tableName As String, ByVal c As CCriteria, ByVal txOrNull As IDbTransaction) As IDbCommand
        ...
        Dim cmd As IDbCommand = Command(sql.ToString, txOrNull)
        cmd.Parameters.Add(Parameter(c.ColumnName, c.ColumnValue))
        Return cmd
    End Function
    ...
    End Function
    Protected Function WhereCriteria(ByVal c As CCriteriaList) As StringBuilder
    ...
    Protected Overloads Sub Parameter(ByVal cmd As IDbCommand, ByVal i As CCriteria)
        ...
        Else
            If i.Sign <> ESign.IN Then 'Not parameterised
    ...
    Private Sub UniqueMarkerNames(ByVal criteria As CCriteriaList, ByVal data As CNameValueList)
        Dim list As New List(Of String)(criteria.Count + data.Count)
        UniqueMarkerNames(list, criteria)
        UniqueMarkerNames(list, data)
    End Sub
    ...
    Private Function ProcessInExpression(ByVal value As Object) As String
        ...
        If TypeOf value Is List(Of Integer) Then
            For Each i As Integer In CType(value, List(Of Integer))
                If sb.Length > 0 Then sb.Append(",")
                sb.Append(i)
            Next
        ElseIf TypeOf value Is List(Of Guid)() Then
            For Each i As Guid In CType(value, List(Of Guid))
                If sb.Length > 0 Then sb.Append(",")
                sb.Append("'").Append(i).Append("'")
            Next
        ElseIf TypeOf value Is List(Of String)() Then
            For Each i As String In CType(value, List(Of String))
                If sb.Length > 0 Then sb.Append(",")
                sb.Append("'").Append(i).Append("'")
            Next
        Else
